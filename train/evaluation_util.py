...
def predict(session, towers, squad_dataset, options):
    """Returns ???"""
    result = _predict(session, towers, squad_dataset,
            options, is_train=False, sample_limit=None)
    return result.context, result.question, result.answer
...
def _predict(session, towers, squad_dataset, options, is_train, sample_limit):
    # passages = []
    # questions = []
    # text_predictions = []
    # ground_truths = []
    ...
    while True:
        ...
        # if sample_limit is not None and total_samples_processed >= sample_limit:
        #     break
        feed_dict = get_predict_feed_dict(squad_dataset, options, towers, is_train=is_train)
        iter_start = time.time()
        towers_spans_values = session.run(run_ops, feed_dict=feed_dict)
        # batch_increment = options.batch_size * max(1, options.num_gpus)
        # total_samples_processed += batch_increment
    ...
    #         ground_truths)
    # f1 = avg_over_list(f1_score, text_predictions, ground_truths)
    return _PredictResult(context, question, answer)
