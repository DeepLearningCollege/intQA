...
def self_critic_rl(options, ce_loss, start_pos_list, end_pos_list):
    ...
    with tf.variable_scope("self_critic_rl"):
        start_pos_list = tf.stack(start_pos_list, axis=1)
        end_pos_list = tf.stack(end_pos_list, axis=1)
        ...
        greedy_start_pos_log_p = tf.multiply(tf.multiply(greedy_start_pos_list, tf.log(start_pos_list)), reward)
        greedy_start_adv = tf.reduce_mean(tf.reduce_sum(-1 * greedy_start_pos_log_p, axis=1))
        greedy_end_pos_log_p = tf.multiply(tf.multiply(greedy_end_pos_list, tf.log(end_pos_list)), reward)
        ...
        norm_rl_loss = tf.multiply(tf.divide(1, tf.multiply(double, sq_sigma_rl)), rl_loss)
        ...
        return loss, rl_loss, sampled_start_pos_list, sampled_end_pos_list, greedy_start_pos_list, greedy_end_pos_list, reward
